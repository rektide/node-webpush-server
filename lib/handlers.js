...
exports.postSub = {
    ...
    handler: function postSub(request, reply) {
        var self = this;
        self.subscribe(request.payload, function afterSub(err, params) {
            if (err) {
            ...
            self.seal(subInfo, function afterSeal(err, subToken) {
                if (err) {
                    reply(boom.wrap(err, 500, 'Error creating subscription'));
                    return;
                }
                ...
                response.created('/s/' + encodeURIComponent(subInfo.id));
...
exports.delUpdate = {
    ...
    handler: function ackUpdate(request, reply) {
        var self = this;
        self.unseal(request.params.updateToken, function afterUnseal(err, updateInfo) {
            if (err) {
                reply(boom.wrap(err, 404, 'Nonexistent update'));
                return;
            }
            if (!updateInfo.receipt) {
            ...
            var receipt = self.receipts.get(updateInfo.receipt.id);
...
exports.postUpdate = {
    ...
    handler: function sendUpdate(request, reply) {
        var self = this;
        self.unseal(request.params.subToken, function afterUnseal(err, subInfo) {
            if (err) {
            ...
            self.unseal(receiptToken, function afterUnseal(err, receiptInfo) {
                if (err) {
                    afterReceipt(boom.wrap(err, 404, 'Invalid receipt URL'));
                    return;
                }
                if (receiptInfo.sub.id != subInfo.id) {
                ...
                afterReceipt(null, subInfo, receiptInfo);
            });
        });
        function afterReceipt(err, subInfo, receiptInfo) {
            if (err) {
                reply(err);
                return;
            }
            ...
            self.seal(updateInfo, function afterSeal(err, updateToken) {
                if (err) {
                    reply(boom.wrap(err, 500, 'Error generating message token'));
                    return;
                }
                var path = '/d/' + encodeURIComponent(updateToken);
                var update = models.Update.fromRequest(request, path);
                // Deliver the message to the client.
                self.push(subInfo.id, subInfo.params, updateInfo.id, update, function afterPush(err) {
                  if (err) {
...
exports.putUpdate = {
    ...
    handler: function putUpdate(request, reply) {
        ...
        self.unseal(request.params.updateToken, function afterUnseal(err, updateInfo) {
            ...
            var update = models.Update.fromRequest(request, request.url.path);
            var subInfo = updateInfo.sub;
            self.push(subInfo.id, subInfo.params, updateInfo.id, update, function afterPush(err) {
                if (err) {
                    reply(boom.wrap(err, 500, 'Error delivering message'));
                    return;
...
exports.getSub = {
    ...
    handler: function getSub(request, reply) {
        ...
        self.flushUpdates(subId, stream, function afterFlush(err) {
            ...
            var wait = headers.getWait(request.headers.prefer);
            if (wait === 0) {
                reply().code(204);
                return;
            }
...
exports.postReceiptSub = {
    ...
    handler: function postReceiptSub(request, reply) {
        ...
        self.unseal(request.params.subToken, function afterUnseal(err, subInfo) {
            ...
            self.seal(receiptInfo, function afterSeal(err, receiptToken) {
                // "Receipt" resource.
...
exports.getReceipts = {
    ...
    handler: function getReceipts(request, reply) {
        ...
        var stream = request.raw.res;
        self.unseal(request.params.receiptToken, function afterUnseal(err, receiptInfo) {
            if (err) {
                reply(boom.wrap(err, 404, 'Nonexistent receipt token'));
                return;
            }
            self.flushAcks(receiptInfo.sub.id, receiptInfo.id, stream, function afterFlush(err) {
                if (err) {
                ...
                self.receipts.add(receiptInfo.id, receipt);
            });
        });
    }
};
